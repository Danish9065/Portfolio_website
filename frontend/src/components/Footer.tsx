    ...
    <footer className="border-t border-[#D7E2EA]/10 bg-[#0C0C0C] py-10">
      <div className="container-shell flex flex-col justify-between gap-6 text-sm text-muted md:flex-row">
        <div className="flex gap-4">
          ...
          {profile?.github_url ? <a href={profile.github_url} target="_blank" rel="noreferrer" aria-label="GitHub"><Github className="h-5 w-5" /></a> : null}
          {profile?.linkedin_url ? <a href={profile.linkedin_url} target="_blank" rel="noreferrer" aria-label="LinkedIn"><Linkedin className="h-5 w-5" /></a> : null}
        </div>
      </div>
    </footer>
  );
